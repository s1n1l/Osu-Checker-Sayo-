...
Two things matter here. Windows auto-repeat sends a key down every ~30 ms
while a key is held, so repeats are filtered by key state. And only keys
from the O3C are recorded, by default only while osu! has focus.
"""
...
class Session:
    ...
    def depth_report(self, trigger_um: int = 500,
        ...
        for key, idx in KEY_TO_INDEX.items():
            ...
            for p in self.presses:
                if p.key != key or p.release is None:
                    continue
    ...
    def peak_depth(self, key: str) -> list[int]:
        """Maximum depth within each press, in micrometres."""
        idx = KEY_TO_INDEX.get(key)
        if idx is None or not self.travel:
            return []
        times = [t for t, _ in self.travel]
        out = []
        for p in self.presses:
